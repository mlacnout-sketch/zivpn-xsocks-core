...
class SecureDnsProxy(
    ...
    private fun resolveViaDot(query: ByteArray): ByteArray {
        ...
        val ssl = sslFactory.createSocket(socket, config.dotHost, config.dotPort, true) as SSLSocket
    ...
    private fun resolveViaDoh(query: ByteArray): ByteArray {
        ...
        val sslFactory = SSLSocketFactory.getDefault() as SSLSocketFactory
        val ssl = sslFactory.createSocket(socket, host, port, true) as SSLSocket
        ssl.useClientMode = true
        ssl.startHandshake()
