...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _checkInitialImport() async {
    try {
      ...
      if (filePath != null && mounted) {
        ...
        if (confirmed == true) {
          final success =
              await BackupRepository().restoreBackup(File(filePath));
          if (success) await _loadData();
        }
      }
    } catch (e) {
      debugPrint('Error checking initial import: $e');
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    if (jsonStr != null) {
      final List<dynamic> jsonData = jsonDecode(jsonStr) as List<dynamic>;
      _accounts = jsonData
          .map((acc) => Account.fromJson(acc as Map<String, dynamic>))
          .toList();
    }
