// File: integration/libuz_wrapper.c
...
static void init_wrapper(void) {
    ...
    // Note: Assuming libuz.so is in LD_LIBRARY_PATH or same directory
    libuz_handle = dlopen("libuz.so", RTLD_NOW);
    if (!libuz_handle) {
        // Fallback to absolute path or just log error
        LOGE("Failed to load libuz.so: %s", dlerror());
        // For testing purposes, we might want to mock if not found,
    ...
    if (!orig_hysteria_connect || !orig_hysteria_send || !orig_hysteria_recv) {
        LOGE("Failed to resolve functions in libuz.so");
        // Don't return, as we might still want the library to load,
        // just wrapper functions will fail gracefully.
    }
